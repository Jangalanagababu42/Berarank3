...
function ValidatorScoring() {
  ...
  const loadMoreValidators = useCallback(async () => {
    ...
    setTimeout(() => {
      const newValidators = Array.from({ length: 10 }, (_, i) => i + 1 + validators.length);
      if (newValidators.length === 0) {
        setHasMore(false);
      } else {
  ...
  return (
    <div className="flex flex-col mb-2 px-4 md:px-0">
      <div className="grid grid-cols-12 space-x-0 md:space-x-5 content-center h-full mb-6 md:mb-10">
        <div className="col-span-12 md:col-start-2 md:col-span-4 font-poppins font-medium text-2xl md:text-[40px] flex items-center mb-4 md:mb-0">
          Validator Scoring
        </div>

        <div className="col-span-6 flex items-center justify-center mb-4 md:mb-0">
          <Menu as="div" className="relative inline-block text-left">
            <MenuButton className="inline-flex justify-center gap-x-5 rounded-[10px] bg-white px-3 py-2 text-grey-stroke text-[16px] font-poppins font-regular text-gray-900 ring-1 ring-inset ring-lightblue-stroke hover:bg-gray-50 w-full md:w-[150px] h-[45px]">
              Score
              <img
                src={dropdown}
                alt="dropdown"
        ...
        <div className="col-span-12 md:col-start-8 md:col-span-4 flex items-center justify-center mb-4 md:mb-0">
          <div className="relative w-full md:w-[303px] h-[47px]">
            <input
              type="text"
              placeholder="Search"
              className="w-full h-full ring-1 ring-inset ring-lightblue-stroke rounded-[10px] px-12 py-3 text-[16px] text-grey-stroke"
            />
            <span className="absolute top-1/2 left-4 transform -translate-y-1/2 text-gray-400">
              <img src={magnify} alt="magnify" className="w-6 h-6" />
            </span>
      ...
      {/* L0vd Score Section */}
      <div className="grid grid-cols-12 space-x-0 md:space-x-5 items-center mb-10">
        <div className="col-span-12 md:col-start-2 md:col-span-9 ring-1 ring-offset ring-light-blue rounded-[20px] p-6 md:p-10 bg-white">
          {/* Rest of the score content */}
        </div>
      </div>
